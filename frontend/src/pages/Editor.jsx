...
const Editor = () => {
  ...
  const [stdin, setStdin] = useState(""); // State for standard input
  ...
  const runProject = () => {
    fetch("https://emkc.org/api/v2/piston/execute", {
      ...
      body: JSON.stringify({
        ...
        stdin: stdin, // Add standard input here
      }),
    })
      .then((res) => res.json())
      .then((data) => {
    ...
    <>
      ...
      >
        ...
        <div className="right p-[15px] w-[50%] h-full bg-[#27272a]">
          ...
          <textarea
            className="w-full h-[15vh] bg-[#1e1e1f] text-white p-2 mt-2 rounded"
            placeholder="Enter standard input here"
            value={stdin}
            onChange={(e) => setStdin(e.target.value)}
          />
          <pre
            className={`w-full h-[60vh] ${error ? "text-red-500" : ""}`}
